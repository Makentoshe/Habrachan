...
class GetPostsByDateConverter: Converter<ResponseBody, Result.GetPostsByDateResponse> {

    override fun convert(value: ResponseBody): Result.GetPostsByDateResponse {
        val json = value.string()
        val success = parseJson(json)
        val error = if (success == null) parseErrorJson(json) else null
        return Result.GetPostsByDateResponse(success = success, error = error)
    }

    private fun parseJson(json: String): GetPostsByDateResult?  {
        val result = Gson().fromJson(json, GetPostsByDateResult::class.java)
