...
class GetPostsByQueryConverter:
    Converter<ResponseBody, Result.GetPostsByQueryResponse> {

    override fun convert(value: ResponseBody): Result.GetPostsByQueryResponse {
        val json = value.string()
        val success = parseJson(json)
        val error = if (success == null) parseErrorJson(json) else null
        return Result.GetPostsByQueryResponse(
            success = success,
            error = error
        )
    }

    private fun parseJson(json: String): GetPostsByQueryResult?  {
        val result = Gson()
            .fromJson(json, GetPostsByQueryResult::class.java)
