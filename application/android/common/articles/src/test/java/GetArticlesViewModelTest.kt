...
class GetArticlesViewModelTest {
    ...
    fun testShouldReturnPagingDataInitially() = runBlocking {
        val viewModel = GetArticlesViewModel(mockUserSession, mockArticlesArena, Option2.Value(getArticleSpec))
        assertNotNull(viewModel.pagingData.firstOrNull())
    }

    @Test
    fun testShouldReturnModelByChannelInvoke() = runBlocking {
        val viewModel = GetArticlesViewModel(mockUserSession, mockArticlesArena, Option2.None)
    ...
    fun testShouldNotReturnModel() = runBlocking {
        assertNull(GetArticlesViewModel(mockUserSession, mockArticlesArena, Option2.None).pagingData.firstOrNull())
    }
