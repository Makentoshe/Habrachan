...
class ArticlesPageItemDecoration(@ColorInt color: Int, private val margin: Float) : ItemDecoration() {

    companion object {
        private const val itemWidth = 2

        fun from(context: Context): ArticlesPageItemDecoration {
            val margin = context.resources.getDimension(R.dimen.dimenMS)
            val color = ContextCompat.getColor(context, R.color.brand_dark)
            return ArticlesPageItemDecoration(color, margin)
        }
    }
