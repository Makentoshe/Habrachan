...
class SourceFirstUsersArenaTest : UsersArenaTest() {
    ...
    fun `test should check loading from cache firstly`() = runBlocking {
        ...
        actualResponse?.user `login should be` "Makentoshe"
    }
    ...
    fun `test should carry source`() = runBlocking {
        val slotGetUserArenaResponse = slot<GetUserArenaResponse>()
        ...
        slotGetUserArenaResponse.captured.user `login should be` "Makentoshe"
